  ...
  return (
    ...
    >
      <div className="cert-card-inner">
        <span className="cert-card-index">{String(cert.id).padStart(2, '0')}</span>
        <div className="cert-image-container">
          <img src={cert.src} alt={cert.title} loading="lazy" className="cert-card-img" />
          <div className="cert-card-overlay">
            <span className="cert-overlay-action">Inspect</span>
          </div>
        </div>
...
export default function CertificationsSection() {
  ...
  return (
    <section className="cert-section" id="certifications" aria-label="Certifications">
      ...
      <div className="cert-sweep cert-sweep-one" />
      <div className="cert-sweep cert-sweep-two" />

      <div className="cert-container">
        <header className="cert-header">
          <span className="cert-kicker">verified growth archive</span>
          <h2 className="cert-title">Certifications</h2>
          <p className="cert-subtitle">
            Credentials mapped across analytics, AI systems, cloud deployment, and full-stack engineering.
          </p>
        </header>

        <div className="cert-showcase" aria-label="Animated certification gallery">
          <div className="cert-showcase-frame" />
          <div className="cert-center-beam" />
            ...
            <div className="cert-marquee-rail">
              <div className="cert-marquee-track cert-track-left">
                {loopRow1.map((cert, index) => (
                  <CertificateCard key={`r1-${cert.id}-${index}`} cert={cert} onSelect={setSelectedCert} />
                ))}
              </div>
            </div>
          </div>
            ...
            <div className="cert-marquee-rail">
              <div className="cert-marquee-track cert-track-right">
                {loopRow2.map((cert, index) => (
                  <CertificateCard key={`r2-${cert.id}-${index}`} cert={cert} onSelect={setSelectedCert} />
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="cert-footer-tip">
          <span>Click any certificate to inspect it in fullscreen view</span>
        </div>
      </div>
            ...
            >
              ...
              <div className="cert-modal-layout">
                <div className="cert-modal-media">
                  <img src={selectedCert.src} alt={selectedCert.title} className="cert-modal-img" />
                </div>
                ...
                <div className="cert-modal-info">
                  ...
                  <p className="cert-modal-description">{selectedCert.details}</p>
